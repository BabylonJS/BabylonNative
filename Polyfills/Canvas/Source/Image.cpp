...
namespace Babylon::Polyfills::Internal
{
    ...
    void NativeCanvasImage::SetSrc(const Napi::CallbackInfo& info, const Napi::Value& value)
    {
        ...
        request.SendAsync().then(m_runtimeScheduler, arcana::cancellation::none(), [env{ info.Env() }, this, request{std::move(request)}](arcana::expected<void, std::exception_ptr> result) {
            if (result.has_error())
            {
            ...
            {
                throw Napi::Error::New(env, "Unable to decode image with provided src for in Canvas.");
            }

            m_width = m_imageContainer->m_width;
            m_height = m_imageContainer->m_height;

            m_onloadHandlerRefs.Call({});
        });
    }
    ...
    void NativeCanvasImage::SetOnload(const Napi::CallbackInfo&, const Napi::Value& value)
    {
        ...
        m_onloadHandlerRefs = Napi::Persistent(eventHandler);
    }
