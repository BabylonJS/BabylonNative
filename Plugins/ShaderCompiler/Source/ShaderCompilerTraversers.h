...
namespace Babylon::ShaderCompilerTraversers
{
    ...
    /// `previousWorld0-3`, `instanceColor`, `splatIndex0-3`) are not fixed to a per-name slot:
    /// each shader's declared set is assigned a dense i_data run (see
    /// AssignBuiltInInstanceSlots), since bgfx requires the used slots to start at i_data0 with
    /// no holes. Names present in `instancedAttributes` are excluded from that run and use the
    /// caller-supplied location instead; an empty map preserves legacy behavior.
