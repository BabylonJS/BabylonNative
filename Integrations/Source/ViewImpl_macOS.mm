...
namespace Babylon::Integrations
{
    std::pair<uint32_t, uint32_t> ViewImpl::QuerySize(Babylon::Graphics::WindowT window)
    {
        ...
        // CAMetalLayer's hosting NSView (e.g. MTKView) is the layer's
        // delegate. If the host added it with Auto Layout and called
        // us before the layout pass ran, force layout now so the
        // layer has valid bounds.
        NSView* hostView = nil;
        if ([layer.delegate isKindOfClass:[NSView class]])
        {
            hostView = (NSView*)layer.delegate;
