...
namespace babylon
{
    ...
    Napi::ObjectReference NativeWindow::Create(Napi::Env& env, void* windowPtr, size_t width, size_t height)
    {
        ...
        Napi::Function constructor = DefineClass(env,
                                                 JS_CLASS_NAME,
                                                 {

                                                 });

        return Napi::Persistent(constructor.New({Napi::External<void>::New(env, windowPtr),
    ...
    {
        return Napi::Persistent(Napi::Function::New(nativeWindow.Env(),
                                                    &NativeWindow::SetTimeout,
                                                    JS_SET_TIMEOUT_NAME,
                                                    NativeWindow::Unwrap(nativeWindow.Value())));
    }
    ...
    void NativeWindow::SetTimeout(const Napi::CallbackInfo& info)
    {
        ...
        nativeWindow.RecursiveWaitOrCall(std::make_shared<Napi::FunctionReference>(std::move(function)),
                                         std::chrono::system_clock::now() + milliseconds);
    }
    ...
    void NativeWindow::RecursiveWaitOrCall(std::shared_ptr<Napi::FunctionReference> function,
