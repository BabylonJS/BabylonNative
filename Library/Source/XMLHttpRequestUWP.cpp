...
namespace babylon
{
    ...
    {
        return SendAsyncImpl().then(
            arcana::inline_scheduler,
            m_runtimeImpl.Cancellation(),
            [url = m_url, responseType = m_responseType, this](arcana::expected<void, std::exception_ptr> result) {
                if (result.has_error())
                {
                    ...
                    // TODO: handle errors
                    return arcana::create_task<std::exception_ptr>(
                               winrt::Windows::Storage::StorageFile::GetFileFromPathAsync(localPath))
                        .then(arcana::inline_scheduler,
                              m_runtimeImpl.Cancellation(),
                              [responseType = std::move(responseType),
                               this](const winrt::Windows::Storage::StorageFile& file) {
                                  if (responseType.empty() || responseType == XMLHttpRequestTypes::ResponseType::Text)
                                  {
                                      return arcana::create_task<std::exception_ptr>(
                                                 winrt::Windows::Storage::FileIO::ReadTextAsync(file))
                                          .then(m_runtimeImpl.Dispatcher(),
                                                m_runtimeImpl.Cancellation(),
                                                [this](const winrt::hstring& text) {
                                  ...
                                  {
                                      return arcana::create_task<std::exception_ptr>(
                                                 winrt::Windows::Storage::FileIO::ReadBufferAsync(file))
                                          .then(m_runtimeImpl.Dispatcher(),
                                                m_runtimeImpl.Cancellation(),
                                                [this](const winrt::Windows::Storage::Streams::IBuffer& buffer) {
                                                    std::byte* bytes;
                                                    auto bufferByteAccess =
                                                        buffer.as<::Windows::Storage::Streams::IBufferByteAccess>();
                                                    winrt::check_hresult(
                                                        bufferByteAccess->Buffer(reinterpret_cast<byte**>(&bytes)));

                                                    m_response = Napi::Persistent(
                                                        Napi::ArrayBuffer::New(Env(), buffer.Length()));
