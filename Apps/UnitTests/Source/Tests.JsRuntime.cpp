// JsRuntimeHost runtime / polyfill tests. Vendored from
// BabylonJS/JsRuntimeHost `Tests/UnitTests/Shared/Shared.cpp` as part of the
// monorepo merge. The suite is named `JsRuntime` rather than upstream's
// `JavaScript` because Tests.JavaScript.cpp already registers
// `TEST(JavaScript, All)` for the Babylon.js/graphics bundle, and gtest rejects
// duplicate suite+name pairs.
...
TEST(JsRuntime, All)
{
    ...
    loader.LoadScript("app:///Assets/tests.jsRuntime.js");
...
// suite, so a throwing timer callback cannot be exercised from tests.jsRuntime.ts.
